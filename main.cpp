...
void loop() {
  ...
  if (POTIS == 1) { 
    int sensorValue_top = analogRead(potiTop)/32;
    ...
    if (abs(hysteresis_top-averageValue_top) > 8){
      ...
      BLEMidiServer.controlChange(midi_channel, 12, hysteresis_top);
    }
    int sensorValue_bottom = analogRead(potiBottom)/32;
    ...
    if (abs(hysteresis_bottom-averageValue_bottom) > 8){
      ...
      BLEMidiServer.controlChange(midi_channel, 12, hysteresis_bottom);
    }
  }
  delay(10);
  }
